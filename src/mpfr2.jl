...
import Base.GMP: ClongMax, CulongMax, CdoubleMax, Limb, libgmp
...
##O#AbstractFloat(x::BigInt) = BigFloat(x)

##O#float(::Type{BigInt}) = BigFloat
...
##O#promote_rule(::Type{BigInt}, ::Type{<:AbstractFloat}) = BigFloat
promote_rule(::Type{BigFloat}, ::Type{<:AbstractFloat}) = BigFloat

##O#big(::Type{<:AbstractFloat}) = BigFloat

##O#big(x::AbstractFloat) = convert(BigFloat, x)
...
for (fJ, fC) in ((:+,:add), (:*,:mul))
    @eval begin
        ...
        function ($fJ)(x::BigFloat, y::BigFloat)
            z = BigFloat(;precision=_proprec($fJ, x, y))
        ...
        function ($fJ)(x::BigFloat, c::CulongMax)
            z = BigFloat(;precision=_proprec($fJ, x, c))
...
function -(c::BigInt, x::BigFloat)
    z = BigFloat(;precision=_proprec(-, x, c))
...
function fma(x::BigFloat, y::BigFloat, z::BigFloat)
    r = BigFloat(;precision=_proprec(fma, x, y, z))
...
function div(x::BigFloat, y::BigFloat)
    z = BigFloat(;precision=_proprec(div, x, y))
...
function div(x::BigFloat, c::CulongMax)
    z = BigFloat(;precision=_proprec(div, x, c))
...
for (fJ, fC, fI) in ((:+, :add, 0), (:*, :mul, 1))
    @eval begin
        function ($fJ)(a::BigFloat, b::BigFloat, c::BigFloat)
            z = BigFloat(;precision=_proprec($fJ, a, b, c))
        ...
        function ($fJ)(a::BigFloat, b::BigFloat, c::BigFloat, d::BigFloat)
            z = BigFloat(;precision=_proprec($fJ, a, b, c, d))
            ccall(($(string(:mpfr_,fC)), libmpfr), Int32, (Ref{BigFloat}, Ref{BigFloat}, Ref{BigFloat}, MPFRRoundingMode), z, a, b, ROUNDING_MODE[])
            ccall(($(string(:mpfr_,fC)), libmpfr), Int32, (Ref{BigFloat}, Ref{BigFloat}, Ref{BigFloat}, MPFRRoundingMode), z, z, c, ROUNDING_MODE[])
        ...
        function ($fJ)(a::BigFloat, b::BigFloat, c::BigFloat, d::BigFloat, e::BigFloat)
            z = BigFloat(;precision=_proprec($fJ, a, b, c, d, e))
            ccall(($(string(:mpfr_,fC)), libmpfr), Int32, (Ref{BigFloat}, Ref{BigFloat}, Ref{BigFloat}, MPFRRoundingMode), z, a, b, ROUNDING_MODE[])
            ccall(($(string(:mpfr_,fC)), libmpfr), Int32, (Ref{BigFloat}, Ref{BigFloat}, Ref{BigFloat}, MPFRRoundingMode), z, z, c, ROUNDING_MODE[])
            ccall(($(string(:mpfr_,fC)), libmpfr), Int32, (Ref{BigFloat}, Ref{BigFloat}, Ref{BigFloat}, MPFRRoundingMode), z, z, d, ROUNDING_MODE[])
...
function -(x::BigFloat)
    z = BigFloat(;precision=_proprec(-, x))
...
function sqrt(x::BigFloat)
    isnan(x) && return x
    z = BigFloat(;precision=_proprec(sqrt, x))
...
##O#sqrt(x::BigInt) = sqrt(BigFloat(x))

function ^(x::BigFloat, y::BigFloat)
    z = BigFloat(;precision=_proprec(^, x, y))
...
for f in (:exp, :exp2, :exp10, :expm1, :cosh, :sinh, :tanh, :sech, :csch, :coth, :cbrt)
    @eval function $f(x::BigFloat)
        z = BigFloat(;precision=_proprec($f, x))
...
function sincos_fast(v::BigFloat)
    precision = _proprec(sincos_fast, v)
    s = BigFloat(;precision)
...
# return log(2)
function big_ln2(;precision=DEFAULT_PRECISION[])
    c = BigFloat(;precision)
...
function ldexp(x::BigFloat, n::Clong)
    z = BigFloat(;precision=_proprec(ldexp, x))
...
ldexp(x::BigFloat, n::Integer) = x * exp2(BigFloat(n;precision=_propre(ldexp, x)))
...
function factorial(x::BigFloat)
    ...
    ui = convert(Culong, x)
    z = BigFloat(;precision=_proprec(factorial, x))
...
function hypot(x::BigFloat, y::BigFloat)
    z = BigFloat(;precision=_proprec(hypot, x, y))
...
function log1p(x::BigFloat)
    ...
    z = BigFloat(;precision=_proprec(log1p, x))
...
function modf(x::BigFloat)
    precision = _proprec(modf, x)
    zint = BigFloat(;precision)
    zfloat = BigFloat(;precision)
...
function rem(x::BigFloat, y::BigFloat)
    z = BigFloat(;precision=_proprec(rem, x, y))
...
rem2pi(x::BigFloat, r::RoundingMode) = rem(x, 2*BigFloat(pi;precision=_propre(rem2pi, x)), r)

function sum(arr::AbstractArray{BigFloat})
    precision = _proprec(sum, arr)
...
function atan(y::BigFloat, x::BigFloat)
    z = BigFloat(;precision=_proprec(atan, x, y))
...
function atand(y::BigFloat, x::BigFloat)
    z = BigFloat(;precision=_proprec(atand, x, y))
...
maxintfloat(x::BigFloat) = BigFloat(2;precision=_propre(maxintfloat, x))^precision(x)
maxintfloat(::Type{BigFloat}) = BigFloat(2)^precision(BigFloat)

function copysign(x::BigFloat, y::BigFloat)
    z = BigFloat(;precision=_proprec(copysign, x))
...
function frexp(x::BigFloat)
    z = BigFloat(;precision=_proprec(frexp, x))
    c = Ref{Clong}()
...
function significand(x::BigFloat)
    z = BigFloat(;precision=_proprec(significand, x))
    c = Ref{Clong}()
    ccall((:mpfr_frexp, libmpfr), Int32, (Ptr{Clong}, Ref{BigFloat}, Ref{BigFloat}, MPFRRoundingMode), c, z, x, ROUNDING_MODE[])
...
typemax(::Type{BigFloat};precision=_precision(BigFloat)) = BigFloat(Inf; precision=precision)
typemin(::Type{BigFloat};precision=_precision(BigFloat)) = BigFloat(-Inf; precision=precision)
...
eps(::Type{BigFloat};precision=_precision(BigFloat)) = nextfloat(BigFloat(1; precision)) - BigFloat(1; precission)

floatmin(::Type{BigFloat};precision=_precision(BigFloat)) = nextfloat(zero(BigFloat;precision))
floatmax(::Type{BigFloat};precision=_precision(BigFloat)) = prevfloat(BigFloat(Inf;precision))
...
function lerpi(j::Integer, d::Integer, a::BigFloat, b::BigFloat)
    t = BigFloat(j;precision=_proprec(lerpi, a, b))/d
    fma(t, b, fma(-t, a, a))
end
...
_proprec(::Function, x, y...) = _precision(BigFloat)
_proprec(::Function, x::BigFloat) = _precision(x)
_proprec(::Function, x::BigFloat, y::BigFloat) = max(_precision(x), _precision(y))
_proprec(::Function, x::BigFloat, y) = _precision(x)
_proprec(::Function, y, x::BigFloat) = _precision(x)
_proprec(-, x::BigFloat) = _precision(x)
_proprec(+, x::BigFloat) = _precision(x)
_proprec(+, a::BigFloat, b::BigFloat) = max(_precision(x), _precision(y))
_proprec(-, a::BigFloat, b::BigFloat) = max(_precision(x), _precision(y))
_proprec(*, a::BigFloat, b::BigFloat) = min(_precision(x) + _precision(y), _precision(BigFloat))
_proprec(/, a::BigFloat, b::BigFloat) = min(_precision(x) + _precision(y), _precision(BigFloat))
