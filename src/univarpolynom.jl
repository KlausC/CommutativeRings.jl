...
function all_exponents(p::P) where P<:UnivariatePolynomial
    expos = findall(!iszero, p.coeff) .+ (ord(p) - 1)
    length(expos) <= 1 && return ord(p), P(LC(p))
    m = minimum(diff(expos))
    m = min(m, ord(p))
    kbest = 0
    best = 0
    for k = 0:m
    ...
    kbest, best, P(p.coeff, ord(p) - kbest)
end
...
function ffactor(p::UnivariatePolynomial)
    #println("ffactor($p)")
    nord, n, q = all_exponents(p)
    n == 1 && return factor(p)
    for kk ∈ unique(factors(n))
        if 1 < kk^2 <= n
            k = n ÷ kk
            r = compress(q, k)
            fr = mfactor(r)
            if !(length(fr) == 1 && last(fr[1]) == 1)
                return cfactors(uncompress.(first.(fr), k) .=> last.(fr))
            end
        end
    end
    factor(p)
...
function cfactors(f::V) where V<:Vector{<:Pair}
    ff = V()
    for (p, ex) in f
        fp = mfactor(p)
