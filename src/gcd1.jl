...
function gz(u::T, v::T) where T<:Integer
...
function gg(u::T, v::T) where T<:Unsigned
    d, xx, yy =
    if u == v
        (u, T(1), T(0))
    elseif iseven(u)
        u2 = u >> 1
        d, x, y = gg(u2, v)
        iseven(x) ? (d, x>>1, y) : (d, (x + v)>>1, y + u2)
    elseif iseven(v)
        v2 = v >> 1
        d, x, y = gg(u, v2)
        iseven(y) ? (d, x, y>>1) : (d, x + v2, (y + u)>>1)
    elseif u > v
        d, x, y = gg(u - v, v)
        (d, x, y + x)
    else
...
Return `-x` if cond == -1 and `x` if cond == 0.
"""
